    ...
    for chunk in dset['flow'].iter_chunks():
        ...
        if out_file is None:
            ...
            for fn in out_file_names:
                h5file = tables.open_file(fn, mode='w')
                out_file.append(h5file)
        ...
        for i, h5file in enumerate(out_file):
            for branch_name, array in branch_splits.items():
                # Handle fixed-size arrays and scalars differently
                if branch_name not in h5file.root._v_children:
                    atom = get_atom(array[i])
                    if isinstance(array[i][0], np.ndarray):
                        # Create an EArray for fixed-size arrays
                        shape = (0,) + array[i].shape[1:]
                        dataset = h5file.create_earray(h5file.root, branch_name, atom, shape)
                        dataset.append(array[i])
                    else:
                    ...
                    dataset = getattr(h5file.root, branch_name)
                    dataset.append(array[i])
        print("Done")
...
if __name__ == "__main__":
    ...
    with open(JSON_file) as file:
        ...
        for key in skim_config:
            print("Running skim with config:", key)
            run(skim_config[key])
        print("Done!")
