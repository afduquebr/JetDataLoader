...
class ParticleFlowNetwork(nn.Module):
    ...
    def forward(self, points, features, lorentz_vectors, mask):
        # x: the feature vector initally read from the data structure, in dimension (N, C, P) for Conv1D
        x = self.input_bn(features)
        # Transpose from (N, C, P) -> (N, P, C) to flatten across per-particle dimensions
        # x = self.input_bn(features.transpose(1, 2))
        # Apply phi layers to each particle
        x = self.phi(x)
        if mask is not None:
            # mask = mask.transpose(1, 2) # If Linear!!!
            x = x * mask.bool().float()
        # Sum over particle dimension (reduce last dimension)
        x = x.sum(-1) # For Conv1D
        # x = x.sum(1)  # For Linear
        return self.fc(x)
